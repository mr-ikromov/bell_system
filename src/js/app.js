...
function fitStage() {
  ...
  el.style.transform = 'translate(-50%,-50%)' + (Math.abs(k - 1) < 0.004 ? '' : ` scale(${k.toFixed(4)})`);
}
...
function buildWheels() {
  ...
  wheelH = createWheel($('#wheelH'), { min: 0, max: 23, value: now.getHours(), enabled: editable, onChange: () => applyTime() });
  wheelM = createWheel($('#wheelM'), { min: 0, max: 59, value: 0, enabled: editable, onChange: () => applyTime() });
}
...
function setBt(st) {
  ...
  chip.querySelector('.bt-txt').textContent = state === 'off' ? t('bt.off') : (st.device || t('bt.on'));
  chip.title = state === 'off' ? t('bt.off') : state === 'muted' ? `${st.device} — ${t('bt.muted')}` : `${st.device} — ${t('bt.on')}`;
}
...
async function buildSpeakers() {
  ...
  menu.innerHTML = list.map(d => `
    <button class="sel__opt${d.selected ? ' on' : ''}${d.usable ? '' : ' off'}" role="option" aria-selected="${!!d.selected}" data-id="${esc(d.id)}">
...
const isTaken = (h, m, exceptId) => S.schedule.some(b => b.id !== exceptId && b.hour === h && b.minute === m);
...
function paintTitle() {
  const cur = current();
  $('#edTitle').textContent = cur ? `${fmt(cur)} — ${cur.label || '—'}` : t('editor.title');
}
...
function bindList() {
  ...
  ['pointerdown', 'wheel', 'keydown'].forEach(ev => $('.panel--editor').addEventListener(ev, bumpAutoLock, { passive: true }));
  const box = $('#bellList');
...
function addBell() {
  ...
  const b = { id: uid(), hour: h, minute: m, label: '', days: current() ? [...current().days] : [...WORK], enabled: true };
  S.schedule.push(b);
  renderAll();
  view(b.id);
  setLocked(false);
...
function renderList() {
  ...
  box.innerHTML = list.map(b => `
    <div class="bell${b.id === S.viewId ? ' is-active' : ''}${b.enabled ? '' : ' is-off'}${nx && nx.bell.id === b.id ? ' is-next' : ''}" data-id="${b.id}">
      ...
      <div class="bell__mid">
        <input class="bell__name" type="text" maxlength="28" placeholder="${t('schedule.name')}" value="${esc(b.label)}" spellcheck="false" />
        <div class="bell__days">${[1,2,3,4,5,6,7].map(d => `<i class="${b.days.includes(d) ? 'on' : ''}"></i>`).join('')}</div>
      </div>
...
const isoDayAfter = (now, off) => isoDay(new Date(now.getFullYear(), now.getMonth(), now.getDate() + off));
...
function tick() {
  ...
  put($('#nextTime'), nx ? fmt(nx.bell) + (nx.offDays ? ` · ${dayShort(isoDayAfter(now, nx.offDays) - 1)}` : '') : '—');
...
document.addEventListener('wheel', e => {if (!e.target.closest('.list__scroll')) e.preventDefault();}, { passive: false });
